...
{
  Addr2Line* self = sys_malloc(sizeof(Addr2Line));
  if (!self)
    return NULL;
  self->executable = sys_malloc(strlen(executable) + 1);
...
//    list_matching_formats(matching);
  ...
  self->syms = (asymbol**)sys_malloc(storage);
...
void addr2line_init()
{
  ...
  for(;;)
  {
	...
	continue;
      ...
      Range* range = sys_malloc(sizeof(Range));
...
static bool addr2line_lookup(Addr2Line* self, bfd_vma pc)
{
  ...
  if (self->found)
  {
	...
	if (len > 1024)
	  len = 1023;
	self->methodName = sys_malloc(len + 1);
