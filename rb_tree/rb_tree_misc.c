...
/*  FUNCTION:  SafeMalloc */
/**/
/*    INPUTS:  size is the size to sys_malloc */
/**/
/*    OUTPUT:  returns pointer to allocated memory if succesful */
/**/
/*    EFFECT:  mallocs new memory.  If sys_malloc fails, prints error message */
...
void * SafeMalloc(size_t size) {
  ...
  if ( (result = sys_malloc(size)) ) { /* assignment intentional */
    return(result);
  } else {
    printf("memory overflow: sys_malloc failed in SafeMalloc.");
